...
public class UserServiceImpl implements IUserService{
    ...
    public ServerResponse<User> login(String username, String password) {
        ...
        if(resultCount == 0){
            return ServerResponse.createByErrorMessage("同户名不存在");
        }
    ...
    public ServerResponse<String> checkValid(String str, String type){
        if(StringUtils.isNotBlank(type)) {
            ...
            if(Const.USERNAME.equals(type)){
                ...
                if(resultCount > 0){
                    return ServerResponse.createByErrorMessage("同户名已存在");
                }
            }
    ...
    public ServerResponse<String> forgetResetPassword(String username, String passwordNew, String forgetToken){
        ...
        ServerResponse validResponse = this.checkValid(username, Const.USERNAME);
        if(validResponse.isSuccess()){
            //用户不存在，找不到username则validResponse返回success
            return ServerResponse.createByErrorMessage("用户不存在");
        }
        ...
        if(StringUtils.equals(forgetToken, token)){
            ...
            return ServerResponse.createByErrorMessage("token错误，情重新获取重置密码的token");
        }
    ...
    public ServerResponse<User> updateInformation(User user){
        //username是不能被更新的
        //email也要进行校验，校验新的email是不是已经存在，并且存在的email如果相同的话，不能是当前的这个用户的
